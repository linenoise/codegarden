...
Takes: C<$self>, C<$uri>, C<$args>, containing optionally 'no_mockup', which will prevent generation of a mockup, or 'headers', containing an array reference of headers to add to the request object
...
sub http_get {
	...
	if ($args && ref($args) eq 'HASH' && $args->{headers} && ref($args->{headers}) eq 'HASH') {
		foreach my $key (keys(%{$args->{headers}})) {
			$request->header( $key => $args->{headers}->{$key} );
		}
	}
