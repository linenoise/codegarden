package Net::Automation::Procedure::ECarList;
...
Net::Automation::Procedure::ECarList - Automation procedure for managing an eCarList application deployment
...
sub meta{
	return {
		...
		methods => {
			test => {'mockup' => {}},
			deploy => {
				### There's only one way we do this
				'eCarList Codebase Deployment' => {
					source => [ 'ssh' ],
					destination => [ 'ssh' ],
				},
			},
